...
function search(opt_options) {
    ...
    if (!nextNode) {
        return options.callback(new Error('Chegaralar ro\'yxati bo\'sh'), options);
    }

    // Iteration check
    options.iteration++;
    if (options.iterationLimit && options.iteration > options.iterationLimit) {
        return options.callback(new Error('Takrorlash chegarasiga yetildi'), options);
    }

    if (window.searchStopped) {
        window.searchStopped = false;
        return options.callback(new Error('Qidirish to\'xtadi'), options);
    }
...
function getNextNode(options) {
    switch (options.type) {
        ...
        case SearchType.A_STAR:
            ...
            return bestNode;
        default:
            throw new Error('Xato qidiruv turi');
    }
}
